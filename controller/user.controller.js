...
exports.createWallet = async (req, res) => {
  try {
    ...
    const checkWallet = await Wallet.findOne({ id: req.params.walletId });
    if (checkWallet) {
      res.status(400).json("wallet already exist");
    }
    ...
    res.status(200).json({ message: "Wallet successfully created", response });
  } catch (error) {
    console.log(error);
